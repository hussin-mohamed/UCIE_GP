...
interface rp_rmblink_bfm (
  ...
  task serialize_data(input logic [pDATA_WIDTH-1:0] _data[pNUM_LANES]
                      , input logic [7:0] _val_stream[], input logic _clk_stream_p[]
                      , input logic _clk_stream_n[], input logic _track_stream[]
    ...
    if (_clk_stream_p.size() != pDATA_WIDTH) begin
      `uvm_fatal("RMBLINK_BFM", $sformatf("Invalid clk_stream_p size: %0d, valid size must be: %0d",
                                          _clk_stream_p.size(), pDATA_WIDTH))
    end
    if (_clk_stream_n.size() != pDATA_WIDTH) begin
      `uvm_fatal("RMBLINK_BFM", $sformatf("Invalid clk_stream_n size: %0d, valid size must be: %0d",
                                          _clk_stream_n.size(), pDATA_WIDTH))
    end
    if (_track_stream.size() != pDATA_WIDTH) begin
      `uvm_fatal("RMBLINK_BFM", $sformatf("Invalid track_stream size: %0d, valid size must be: %0d",
                                          _track_stream.size(), pDATA_WIDTH))
    end
  ...
  task serialize_valid_pattern(input logic [7:0] _val_stream[], input logic _clk_stream_p[]
                               , input logic _clk_stream_n[], input logic _track_stream[]);
    if (_clk_stream_p.size() != _clk_stream_n.size()) begin
      `uvm_fatal("CLK_STREAM_SIZE_MISMATCH",
                 "Clock stream arrays must be of the same size in serialize_valid_pattern.")
    end

    if (_clk_stream_p.size() != CLK_STREAM_LEN_VALID_PAT || _clk_stream_n.size() != CLK_STREAM_LEN_VALID_PAT) begin
      `uvm_fatal(
          "CLK_STREAM_SIZE_HR",
          "Clock stream arrays must have exactly {CLK_STREAM_LEN_VALID_PAT} elements for Half Rate (HR) mode in serialize_valid_pattern.")
    end

    if (_val_stream.size() != VALID_CLK_PATTERN_STREAM_LEN) begin
      `uvm_fatal(
          "VALID_STREAM_SIZE_MISMATCH",
          "Valid stream arrays must be of size {VALID_CLK_PATTERN_STREAM_LEN} in serialize_valid_pattern.")
    end

    for (int t = 0; t < $size(_val_stream, 1); t++) begin
      for (int i = 0; i < $size(_val_stream, 2); i++) begin
        @(posedge i_dclk) i_valid <= _val_stream[t][i];
        @(negedge i_dclk) i_clk_p <= _clk_stream_p[(t*8)+i];
  ...
  task serialize_clk_pattern(input logic _clk_stream_p[], input logic _clk_stream_n[]
                             , input logic _track_stream[], input int unsigned _idle_ui_cnt);
    if (_clk_stream_p.size() != _clk_stream_n.size()) begin
      `uvm_fatal("CLK_STREAM_SIZE_MISMATCH",
                 "Clock stream arrays must be of the same size in serialize_clk_pattern.")
    end

    if (_clk_stream_p.size() != CLK_STREAM_LEN_CLK_PAT  || _clk_stream_n.size() != CLK_STREAM_LEN_CLK_PAT) begin
      `uvm_fatal(
          "CLK_STREAM_SIZE_HR",
          "Clock stream arrays must have exactly {CLK_STREAM_LEN_CLK_PAT} elements for Half Rate (HR) mode in serialize_clk_pattern.")
    end

    for (int t = 0; t < VALID_CLK_PATTERN_STREAM_LEN; t++) begin
      for (int i = 0; i < CLK_STROBE_CLK_PAT; i++) begin
        @(posedge i_dclk) i_clk_p <= _clk_stream_p[(t*CLK_STROBE_CLK_PAT)+i];
