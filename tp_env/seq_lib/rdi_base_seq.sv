...
`else
    ...
    `uvm_info("RDI_SEQ", "Waiting for ACTIVE state on pl_state_sts...", UVM_LOW)

      forever begin
        @(posedge vif.clk);
        if (vif.pl_state_sts == 4'b0001 || vif.pl_state_sts == 4'b0011) begin // PL_STS_ACTIVE or PLS_STS_PMNAK
          `uvm_info("RDI_SEQ", "ACTIVE state detected on pl_state_sts — starting flit generation", UVM_LOW)
          break;
        end
      end
`endif
...
`else
        if (vif.pl_state_sts != 4'b0001 && vif.pl_state_sts != 4'b0011) begin
          `uvm_info("RDI_SEQ", $sformatf("State changed (pl_state_sts=0x%0h) — halting flit gen after %0d flits",
                    vif.pl_state_sts, i), UVM_LOW)
          break;
        end
`endif
