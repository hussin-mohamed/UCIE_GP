...
module ucie_timeout_timer #(
    ...
    logic [DECODING_WIDTH-1:0] r_rx_encoding_prev;
    ...
    always_ff @(posedge i_clk or posedge i_reset) begin
        if(i_reset) begin
             r_rx_encoding_prev <= '0;
        end else begin
            r_rx_encoding_prev <= o_rx_encoding;
        end
    end

    // If current input != previous stored input, state changed.
    assign w_state_changed = (r_rx_encoding_prev != o_rx_encoding);
    ...
    `ifdef SIM
        ...
        property clear_on_state_change(counter);
            @(posedge i_clk) disable iff(i_reset)
            r_rx_encoding_prev != $past(r_rx_encoding_prev) |-> (counter == 0); 
        endproperty
