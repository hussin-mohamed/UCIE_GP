...
module ucie_timeout_timer #(
  ...
  // Simulation thresholds (dynamic, based on clock speed from i_speedreg)
  logic [12:0] SIM_CYCLES_8MS;
  logic [12:0] SIM_CYCLES_4MS;
  logic [12:0] SIM_CYCLES_1MS;
  logic [12:0] SIM_CYCLES_1US;
  logic [12:0] SIM_CYCLES_2US;

  always_comb begin : sim_threshold_calc
      case (i_speedreg)
          3'b000: SIM_CYCLES_8MS = CYCLES;
          3'b001: SIM_CYCLES_8MS = CYCLES * 2;
          3'b010: SIM_CYCLES_8MS = CYCLES * 3;
          3'b011: SIM_CYCLES_8MS = CYCLES * 4;
          3'b100: SIM_CYCLES_8MS = CYCLES * 6;
          3'b101: SIM_CYCLES_8MS = CYCLES * 8;
          default: SIM_CYCLES_8MS = CYCLES; // Default to the lowest frequency clock
      endcase
  end
  assign SIM_CYCLES_4MS = SIM_CYCLES_8MS / 2;
  assign SIM_CYCLES_1MS = SIM_CYCLES_8MS / 8;
  assign SIM_CYCLES_1US = SIM_CYCLES_8MS / 8000;
  assign SIM_CYCLES_2US = SIM_CYCLES_8MS / 4000;
  ...
  always_ff @(posedge i_clk or posedge i_reset) begin
    if (i_reset) begin
      ...
      r_counter_1us <= '0;
      r_counter_2us <= '0;
    end else if (w_state_changed) begin
      r_counter_1ms <= '0;
      r_counter_4ms <= '0;
      r_counter_8ms <= '0;
      r_counter_1us <= '0;
      ...
      if (r_counter_8ms >= CYCLES_8MS - 1) r_counter_8ms <= '0;
      else r_counter_8ms <= r_counter_8ms + 1'b1;

      // 4ms counter — wraps after each period
      if (r_counter_4ms >= CYCLES_4MS - 1) r_counter_4ms <= '0;
      else r_counter_4ms <= r_counter_4ms + 1'b1;

      // 1ms counter — wraps after each period
      if (r_counter_1ms >= CYCLES_1MS - 1) r_counter_1ms <= '0;
      else r_counter_1ms <= r_counter_1ms + 1'b1;

      // 2us counter — wraps after each period
      if (r_counter_2us >= CYCLES_2US - 1) r_counter_2us <= '0;
      else r_counter_2us <= r_counter_2us + 1'b1;

      // 1us counter — wraps after each period
      if (wait_1us) begin
        if (r_counter_1us >= CYCLES_1US - 1) r_counter_1us <= '0;
        else r_counter_1us <= r_counter_1us + 1'b1;
