...
module ucie_ltsm_rx_mbinit_reversal #(
    ...
    input                      o_timer_8ms,
    input [    DATA_WIDTH-1:0] i_rx_data_results, // from pattern detector
  ...
  logic [DATA_WIDTH-1:0] i_rx_data_results_reg;

  // -------------------------------------------------------------------------
  // Combinational popcount of i_rx_data_results_reg[15:0]
  //   5 bits — maximum value 16.
  //   count <= 8: majority of lanes bad → TX will apply reversal → loop to CLEAR_LOG
  //   count >  8: majority good        → TX proceeds to done    → go to DONE_HANDSHAKE
  // -------------------------------------------------------------------------
  logic [           4:0] w_popcount;

  always_comb begin
    w_popcount = '0;
    for (int i = 0; i < 16; i++) w_popcount = w_popcount + {4'b0, i_rx_data_results_reg[i]};
  end
  ...
  always_ff @(posedge i_clk or posedge i_reset) begin
    if (i_reset) begin
      ...
      substates_done        <= 0;
      i_rx_data_results_reg <= '0;
    end else if (i_current_state != MBINIT_REVERSAL) begin
      current_substate      <= INIT_HANDSHAKE;
      substates_done        <= 0;
      i_rx_data_results_reg <= '0;
    end else begin
      if (current_substate == DONE_HANDSHAKE && i_sb_rx_req && i_rx_decoding == 9'h35) begin
        substates_done   <= 1;
  ...
  always_comb begin
    ...
    if (!substates_done && o_timer_8ms) begin
      ...
      case (current_substate)
        ...
        // --------------------------------------------------------------
        // INIT_HANDSHAKE (0x30)
        // Wait for TX REQ 0x30; send RSP immediately (done_ack controls
        // when RSP drops after sideband confirms delivery).
        // --------------------------------------------------------------
        ...
        // Re-entered after reversal is applied by TX.
        // --------------------------------------------------------------
        ...
        // RX performs lane ID detection. Advance when hardware signals done.
        ...
        // --------------------------------------------------------------
        // SEND_RESP (0x33)
        // Send lane detection results to TX via o_rx_data.
        // When i_sb_rx_done confirms the RSP was sent, evaluate w_popcount
        // to mirror TX's decision:
        //   count <= 8: TX applies reversal → loop back to CLEAR_LOG_HANDSHAKE
...
`ifdef SIM
  ...
  ENC_INIT_HANDSHAKE :
  assert property (enc_check(INIT_HANDSHAKE, 9'h30))
  else $error("ASSERT FAIL [ENC_INIT_HANDSHAKE]: wrong encoding");
  ENC_CLEAR_LOG_HANDSHAKE :
  assert property (enc_check(CLEAR_LOG_HANDSHAKE, 9'h31))
  else $error("ASSERT FAIL [ENC_CLEAR_LOG_HANDSHAKE]: wrong encoding");
  ENC_LANE_ID_DETECTION :
  assert property (enc_check(LANE_ID_DETECTION, 9'h32))
  else $error("ASSERT FAIL [ENC_LANE_ID_DETECTION]: wrong encoding");
  ENC_WAIT_RESULT_REQ :
  assert property (enc_check(WAIT_RESULT_REQ, 9'h33))
  else $error("ASSERT FAIL [ENC_WAIT_RESULT_REQ]: wrong encoding");
  ENC_SEND_RESP :
  assert property (enc_check(SEND_RESP, 9'h33))
  else $error("ASSERT FAIL [ENC_SEND_RESP]: wrong encoding");
  ENC_DONE_HANDSHAKE :
  assert property (enc_check(DONE_HANDSHAKE, 9'h35))
  else $error("ASSERT FAIL [ENC_DONE_HANDSHAKE]: wrong encoding");

  property timeout_error;
    @(posedge i_clk) disable iff (i_reset) o_timer_8ms |-> o_train_error;
  endproperty
  TIMEOUT_TRAIN_ERROR :
  assert property (timeout_error)
  else $error("ASSERT FAIL [TIMEOUT_TRAIN_ERROR]: train_error not set on timeout");

  property timeout_reset_sub;
    @(posedge i_clk) disable iff (i_reset) o_timer_8ms |=> current_substate == INIT_HANDSHAKE;
  endproperty
  TIMEOUT_RESETS_SUBSTATE :
  assert property (timeout_reset_sub)
  else $error("ASSERT FAIL [TIMEOUT_RESETS_SUBSTATE]: substate not reset after timeout");

  // count <= 8 → loop back to CLEAR_LOG
  property reversal_loops_back;
    @(posedge i_clk) disable iff (i_reset || o_timer_8ms)
        (i_current_state == MBINIT_REVERSAL &&
         current_substate == SEND_RESP &&
         i_sb_rx_done && w_popcount <= 5'd8)
        |=> current_substate == CLEAR_LOG_HANDSHAKE;
  endproperty
  REVERSAL_LOOPS_BACK :
  assert property (reversal_loops_back)
  else $error("ASSERT FAIL [REVERSAL_LOOPS_BACK]: did not loop to CLEAR_LOG when popcount<=8");
  ...
  endproperty
  REVERSAL_NOT_NEEDED :
  assert property (reversal_not_needed)
