...
module ucie_ltsm_rx_mbinit_repairmb #(
    ...
    ucie_RX_Data_to_Clock_eye_sweep ucie_RX_Data_to_Clock_eye_sweep_inst (
        ...
        .init             (1'b0),
        .no_retry         (1'b0),
        .result           ({56'h0, i_rx_sweep_result}),
    ...
    always_comb begin
        ...
        if (o_timer_8ms || train_error_sweep) begin
            ...
            case (current_substate)
                ...
                // TX sends degrade REQ 0x3A with its lane map in i_rx_info[2:0].
                // Match (w_extracted_lane_map == r_lane_map) → SEND_RESP
                // Mismatch                                  → DEGRADE, update r_lane_map
