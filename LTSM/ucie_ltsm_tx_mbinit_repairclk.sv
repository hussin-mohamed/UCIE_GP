module ucie_ltsm_tx_mbinit_repairclk #(
...
`ifdef SIM
    ...
    ENC_INIT_HANDSHAKE    : assert property (encoding_check(INIT_HANDSHAKE,    9'h20))
        else $error("ASSERT FAIL [ENC_INIT_HANDSHAKE]: wrong encoding in INIT_HANDSHAKE");
    ENC_PATTERN_GENERATION: assert property (encoding_check(PATTERN_GENERATION, 9'h21))
        else $error("ASSERT FAIL [ENC_PATTERN_GENERATION]: wrong encoding in PATTERN_GENERATION");
    ENC_RESULT_HANDSHAKE  : assert property (encoding_check(RESULT_HANDSHAKE,  9'h22))
        else $error("ASSERT FAIL [ENC_RESULT_HANDSHAKE]: wrong encoding in RESULT_HANDSHAKE");
    ENC_DONE_HANDSHAKE    : assert property (encoding_check(DONE_HANDSHAKE,    9'h23))
    ...
    // --------------------------------------------------------------------------
    // Done asserted as combinational pulse on RSP + correct decoding in DONE_HANDSHAKE
    // --------------------------------------------------------------------------
    property done_on_rsp;
        @(posedge i_clk) disable iff (i_reset || o_timer_8ms)
        (i_current_state == MBINIT_REPAIRCLK &&
         current_substate == DONE_HANDSHAKE &&
         i_sb_tx_rsp && i_tx_decoding == 9'h23)
        |-> o_done_mbinit_repairclk_tx;
    endproperty
    DONE_REPAIRCLK : assert property (done_on_rsp)
        else $error("ASSERT FAIL [DONE_REPAIRCLK]: done not asserted on RSP + 0x23");
    ...
    property req_raised_when_needed;
        @(posedge i_clk) disable iff (i_reset || o_timer_8ms)
        (i_current_state == MBINIT_REPAIRCLK && !done_ack && !substates_done)
        |-> o_tx_sb_req;
    endproperty
    ...
    property req_dropped_after_done_ack;
        @(posedge i_clk) disable iff (i_reset || o_timer_8ms)
        (i_current_state == MBINIT_REPAIRCLK && done_ack)
        |-> !o_tx_sb_req;
    endproperty
    ...
    // --------------------------------------------------------------------------
    // Done never asserts outside MBINIT_REPAIRCLK
    // --------------------------------------------------------------------------
    property done_only_in_state;
        @(posedge i_clk) disable iff (i_reset)
        i_current_state != MBINIT_REPAIRCLK |-> !o_done_mbinit_repairclk_tx;
    endproperty
