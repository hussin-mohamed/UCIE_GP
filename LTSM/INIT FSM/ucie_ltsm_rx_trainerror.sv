...
always_comb begin 
    ...
    if(o_timer_8ms == 1) begin 
    ...
    else if(i_current_state == TRAINERROR && substates_done == 0) begin
        case(current_substate)
            ENTRY_HANDSHAKE: begin 
                ...
                if(i_sb_rx_req && i_rx_decoding == 9'h40) begin 
                    req_sent = 1;
                end 

                // Next State Logic
                if(i_sb_rx_done && i_rx_decoding == 9'h40 && req_sent) begin 
                    next_substate = RX_TRAINERROR;
                end 
            end
...
`ifdef SIM
    ...
    property done_on_rsp;
        ...
        (i_current_state == TRAINERROR &&
         ...
         !i_lp_linkerror && i_sb_cur_msg_done && req_sent)
