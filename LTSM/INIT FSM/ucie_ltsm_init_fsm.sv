...
module ucie_ltsm_init_fsm #(
    ...
    // -------------------------------------------------------------------------
    input  logic [DATA_WIDTH-1:0]       i_rx_repairclk_pattern_results,
    input  logic [DATA_WIDTH-1:0]       i_rx_repairval_pattern_results,
    ...
    always_ff @(posedge i_clk or posedge i_reset) begin
        if (i_reset) begin
            done_tx_reset      <= 0; done_rx_reset      <= 0;
            done_tx_sbinit     <= 0; done_rx_sbinit     <= 0;
            done_tx_param      <= 0; done_rx_param      <= 0;
            done_tx_cal        <= 0; done_rx_cal        <= 0;
            done_tx_repairclk  <= 0; done_rx_repairclk  <= 0;
            done_tx_repairval  <= 0; done_rx_repairval  <= 0;
            done_tx_reversal   <= 0; done_rx_reversal   <= 0;
            done_tx_repairmb   <= 0; done_rx_repairmb   <= 0;
            done_tx_trainerror <= 0; done_rx_trainerror <= 0;
        end else begin
            // ---- RESET ----
    ...
    always_ff @(posedge i_clk or posedge i_reset) begin
        if (i_reset)
            init_train_en_reg <= 0;
        else if (current_state == MBINIT_REPAIRMB &&
                 done_tx_repairmb && done_rx_repairmb)
            init_train_en_reg <= 1;
        else if (current_state != MBINIT_REPAIRMB)
            init_train_en_reg <= 0;
    end
    ...
    always_comb begin
        ...
        if (any_sbinit_error && current_state == SBINIT) begin
        ...
        else if (any_other_error) begin
            next_state = TRAINERROR;
        end
        else if (init_train_en_reg && i_train_active_error) begin
            next_state = TRAINERROR;
        end
        else begin
            // Normal sequencing
            case (current_state)
                RESET: begin
                    if (done_tx_reset && done_rx_reset)
                        next_state = SBINIT;
                end

                SBINIT: begin
                    if (done_tx_sbinit && done_rx_sbinit)
                        next_state = MBINIT_PARAM;
                end

                MBINIT_PARAM: begin
                    if (done_tx_param && done_rx_param)
                        next_state = MBINIT_CAL;
                end

                MBINIT_CAL: begin
                    if (done_tx_cal && done_rx_cal)
                        next_state = MBINIT_REPAIRCLK;
                end

                MBINIT_REPAIRCLK: begin
                    if (done_tx_repairclk && done_rx_repairclk)
                        next_state = MBINIT_REPAIRVAL;
                end

                MBINIT_REPAIRVAL: begin
                    if (done_tx_repairval && done_rx_repairval)
                        next_state = MBINIT_REVERSAL;
                end

                MBINIT_REVERSAL: begin
                    if (done_tx_reversal && done_rx_reversal)
                ...
                TRAINERROR: begin
                    if (done_tx_trainerror && done_rx_trainerror)
                        next_state = RESET;
                end
    ...
    // ----- TX RESET -----
    ucie_ltsm_tx_reset #(.DECODING_WIDTH(DECODING_WIDTH)) u_tx_reset (
        .i_clk            (i_clk),
        .i_reset          (i_reset),
    ...
    // ----- RX RESET -----
    ucie_ltsm_rx_reset #(.DECODING_WIDTH(DECODING_WIDTH)) u_rx_reset (
        .i_clk            (i_clk),
        .i_reset          (i_reset),
    ...
    ucie_ltsm_rx_mbinit_repairclk #(
        ...
        .i_current_state          (current_state),
        .o_timer_8ms              (o_timer_8ms),
        .i_pattern_detection_results(i_rx_repairclk_pattern_results),
    ...
    ucie_ltsm_rx_mbinit_repairval #(
        ...
        .i_current_state          (current_state),
        .o_timer_8ms              (o_timer_8ms),
        .i_pattern_detection_results(i_rx_repairval_pattern_results),
    ...
    ucie_ltsm_rx_mbinit_reversal #(
        ...
        .i_current_state          (current_state),
        .o_timer_8ms              (o_timer_8ms),
        .i_pattern_detection_results(i_rx_reversal_pattern_results),
    ...
    always_comb begin
        ...
        o_rx_sb_rsp = o_rx_sb_rsp; // already assigned above in the MUX
    end
...
`ifdef SIM
    ...
    endproperty
    VALID_STATE : assert property (valid_state)
    ...
    property sbinit_error_to_reset;
        ...
        |=> current_state == RESET;
    endproperty
    SBINIT_ERR_TO_RESET : assert property (sbinit_error_to_reset)
    ...
    endproperty
    OTHER_ERR_TO_TRAINERROR : assert property (other_error_to_trainerror)
    ...
    property trainerror_done_to_reset;
        ...
        |=> current_state == RESET;
    endproperty
    TRAINERROR_TO_RESET : assert property (trainerror_done_to_reset)
    ...
    property init_train_en_gated;
        @(posedge i_clk) disable iff (i_reset)
        o_init_train_en |-> current_state == MBINIT_REPAIRMB;
    endproperty
    INIT_TRAIN_EN_GATED : assert property (init_train_en_gated)
    ...
    property active_error_to_trainerror;
        ...
        |=> current_state == TRAINERROR;
    endproperty
    ACTIVE_ERR_TO_TRAINERROR : assert property (active_error_to_trainerror)
        else $error("ASSERT FAIL [ACTIVE_ERR_TO_TRAINERROR]: did not enter TRAINERROR on active training error");
