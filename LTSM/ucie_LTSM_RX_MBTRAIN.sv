          ...
          if (!substates_done) begin
            case (current_substate)
              ...
              0: begin
                ...
                else if (i_rx_decoding == 'h80 && i_sb_rx_req) o_rx_sb_rsp_reg = 1;
...
`ifdef ASSERT_ON
  ...
  property done_ack_assert_property;
    @(posedge i_clk) disable iff (i_reset) i_sb_rx_done |-> done_ack;
  endproperty
  ...
  property previous_state_done_property;
    @(posedge i_clk) disable iff (i_reset) (rsp_received && rsp_sent) |-> previous_state_done;
  endproperty

  property reset_state_property;
    @(posedge i_clk) i_reset |=> (CS == VALVREF);
  endproperty

  property reset_substate_property;
    @(posedge i_clk) i_reset |=> (current_substate == 0);
  endproperty

  property reset_encoding_property;
    @(posedge i_clk) i_reset |=> (o_rx_encoding == 0);
  endproperty

  property reset_data_property;
    @(posedge i_clk) i_reset |=> (o_rx_data == 0);
  endproperty

  property reset_info_property;
    @(posedge i_clk) i_reset |=> (o_rx_info == 0);
  endproperty

  property reset_sb_req_property;
    @(posedge i_clk) i_reset |=> (o_rx_sb_req == 0);
  endproperty

  property reset_sb_rsp_property;
    @(posedge i_clk) i_reset |=> (o_rx_sb_rsp == 0);
  endproperty


  property reset_sb_done_property;
    @(posedge i_clk) i_reset |=> (o_rx_sb_done == 0);
  endproperty


  property reset_req_received_property;
    @(posedge i_clk) i_reset |=> (req_received == 0);
  endproperty

  property reset_encoding_req_received_property;
    @(posedge i_clk) i_reset |=> (encoding_req_received == 0);
  endproperty

  property sb_done_self_clearing_property;
    @(posedge i_clk) disable iff (i_reset) o_rx_sb_done |=> (!o_rx_sb_done);
  endproperty

  property sb_done_assert_on_req_property;
    @(posedge i_clk) disable iff (i_reset) (i_sb_rx_req && !o_rx_sb_done) |=> o_rx_sb_done;
  endproperty

  property sb_done_assert_on_rsp_property;
    @(posedge i_clk) disable iff (i_reset) (i_sb_rx_rsp && !o_rx_sb_done) |=> o_rx_sb_done;
  endproperty

  property not_state_enb_resets_to_valvref_property;
    @(posedge i_clk) disable iff (i_reset) (!state_enb) |=> (CS == VALVREF);
  endproperty

  property not_state_enb_resets_substate_property;
    @(posedge i_clk) disable iff (i_reset) (!state_enb) |=> (current_substate == 0);
  endproperty
  ...
  property previous_state_done_reset_property;
    @(posedge i_clk) i_reset |-> !previous_state_done;
  endproperty

  property mutual_exclusive_req_rsp_property;
    @(posedge i_clk) disable iff (i_reset) !(o_rx_sb_req && o_rx_sb_rsp);
  endproperty
  ...
  property req_received_captures_encoding_property;
    @(posedge i_clk) disable iff (i_reset)
        i_sb_rx_req |=> (req_received && encoding_req_received == $past(
        i_rx_decoding
    ));
  endproperty
  ...
  property substates_done_clears_on_transition_property;
    @(posedge i_clk) disable iff (i_reset) (CS != $past(
        CS
    )) |-> !substates_done;
  endproperty
  ...
  done_ack_assert_assertion :
  assert property (done_ack_assert_property)
  else $error("Assertion failed: done_ack should be set when i_sb_rx_done is asserted");
  cover property (done_ack_assert_property);

  done_ack_deassert_assertion :
  assert property (done_ack_deassert_property)
  else $error("Assertion failed: done_ack should be low when i_sb_rx_rsp is asserted");
  cover property (done_ack_deassert_property);

  previous_state_done_assertion :
  assert property (previous_state_done_property)
  else
    $error(
        "Assertion failed: previous_state_done should be set when both rsp_received and rsp_sent are true"
    );
  ...
  sb_done_self_clearing_assertion :
  assert property (sb_done_self_clearing_property)
  ...
  speedidle_to_txselfcal_assertion :
  assert property (speedidle_to_txselfcal_property)
  ...
  txselfcal_to_rxclkcal_assertion :
  assert property (txselfcal_to_rxclkcal_property)
  ...
  rxclkcal_to_valtraincenter_assertion :
  assert property (rxclkcal_to_valtraincenter_property)
  ...
  valtraincenter_to_valtrainvref_assertion :
  assert property (valtraincenter_to_valtrainvref_property)
  ...
  valtrainvref_to_datatraincenter1_assertion :
  assert property (valtrainvref_to_datatraincenter1_property)
  ...
  datatraincenter1_to_datatrainvref_assertion :
  assert property (datatraincenter1_to_datatrainvref_property)
  ...
  datatrainvref_to_rxdeskew_assertion :
  assert property (datatrainvref_to_rxdeskew_property)
  ...
  rxdeskew_to_datatraincenter2_assertion :
  assert property (rxdeskew_to_datatraincenter2_property)
  ...
  datatraincenter2_completes_training_assertion :
  assert property (datatraincenter2_completes_training_property)
  else
    $error(
        "Assertion failed: (train_link_init_en || train_phyretrain_en) should be set when DATATRAINCENTER2 completes"
    );
  cover property (datatraincenter2_completes_training_property);

  previous_state_done_reset_assertion :
  assert property (previous_state_done_reset_property)
  ...
  mutual_exclusive_req_rsp_assertion :
  assert property (mutual_exclusive_req_rsp_property)
  else
    $error("Assertion failed: o_rx_sb_req and o_rx_sb_rsp should not be asserted simultaneously");
  cover property (mutual_exclusive_req_rsp_property);

  clock_test_enable_only_in_substate1_valvref_assertion :
  assert property (clock_test_enable_only_in_substate1_valvref_property)
  ...
  txselfcal_waits_for_sb_done_assertion :
  assert property (txselfcal_waits_for_sb_done_property)
  else
    $error(
        "Assertion failed: TXSELFCAL should remain in substate 0 until i_sb_rx_done is asserted"
    );
  cover property (txselfcal_waits_for_sb_done_property);

  req_received_captures_encoding_assertion :
  assert property (req_received_captures_encoding_property)
  else
    $error(
        "Assertion failed: req_received should be set and encoding_req_received should capture i_rx_decoding when i_sb_rx_req is asserted"
    );
  cover property (req_received_captures_encoding_property);

  valid_state_assertion :
  assert property (valid_state_property)
  else $error("Assertion failed: CS contains an invalid state value");
  cover property (valid_state_property);

  substates_done_clears_on_transition_assertion :
  assert property (substates_done_clears_on_transition_property)
  ...
  state_transition_values_assertion :
  assert property (state_transition_values_property)
  else
    $error(
        "Assertion failed: on state transition, substates_done should be 0, o_rx_sb_req should be 0, and o_rx_sb_rsp should be 0"
    );
