...
module ucie_ltsm_rx_mbinit_repairclk #(
    ...
    // Pattern detection results from RX PHY path (4 clock lane health bits)
    ...
    always_ff @(posedge i_clk or posedge i_reset) begin
        if (i_reset || i_current_state != MBINIT_REPAIRCLK) begin
            ...
            // Latch substates_done when DONE_HANDSHAKE completes
            if (current_substate == DONE_HANDSHAKE &&
                i_sb_rx_req && i_rx_decoding == 9'h24)
                substates_done <= 1;
        end
    end
    ...
    always_comb begin
        ...
        if (o_timer_8ms) begin
        ...
        else if (i_current_state == MBINIT_REPAIRCLK && !substates_done) begin
            case (current_substate)
                ...
                INIT_HANDSHAKE: begin
                    ...
                    if (i_sb_rx_req && i_rx_decoding == 9'h20)
                ...
                SEND_RESP: begin
                    o_rx_encoding   = 9'h23;
                    o_rx_info[3:0]  = i_pattern_detection_results_reg[3:0];
                    o_rx_sb_rsp     = 1;
