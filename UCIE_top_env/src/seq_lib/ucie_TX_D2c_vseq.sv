...
class ucie_TX_D2C_vseq extends ucie_vseq_base;
  ...
  virtual task body();
    ...
    if (data_mode == VALID_PATTERN) begin
      `uvm_fatal("SEQ_CFG_ERR", "There is no VALID pattern for TX D2C, please choose a different data pattern!")
    end

    if (D2c_mode == LOOP_TILL_ERROR) begin
      `uvm_fatal("SEQ_CFG_ERR", "There is no Loop for TX D2C, please choose a different data pattern!")
    end
    ...
    sb_ltsm_item.set_rx_encoding(sb_shared_pkg::Data_To_Clock_test_TX_TX_INIT_Handshake);
    send_sb_msg(sb_ltsm_item);
