...
class ucie_sbinit_vseq extends ucie_vseq_base;
  ...
  virtual task body();
    ...
    if (sbinit_fail_cnt == 0) begin
    ...
    end else if (sbinit_fail_cnt == 1) begin
      ...
      sbinit_phylink_random_seq.configure(._sbinit_seq_mode(RAND_TILL_DETECTION));
      sbinit_phylink_random_seq.start(sb_phylink_seqr);
      sbinit_bringup_vseq.configure(._msg_drop_mode(DROP_DONE_RSP));
      sbinit_bringup_vseq.start(p_sequencer);
    end else if (sbinit_fail_cnt == 4) begin
      `uvm_info(get_type_name(), $sformatf("Executing Attempt 5: sbinit_fail_cnt = %0d. Configuring RAND_TILL_DETECTION and NO_DROP.", sbinit_fail_cnt), UVM_MEDIUM)
      sbinit_fail_cnt++;
      sbinit_phylink_random_seq.configure(._sbinit_seq_mode(RAND_TILL_DETECTION));
      sbinit_phylink_random_seq.start(sb_phylink_seqr);
