...
class mbtrain_rx_txselfcal extends state;
    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_sequence_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        if( item_rx_fsm_sb_in.i_rx_decoding == RX_MBTRAIN_TXSELFCAL_End_Handshake && item_rx_fsm_sb_in.i_sb_tx_req==1'b1 && cntxt.currentstate_rx == mbtrain_tx_speedidle::instance())begin
            o_tx_encoding_expected = RX_MBTRAIN_TXSELFCAL_End_Handshake;
            ...
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_sb_tx_req_expected == item_tx_fsm_sb_out.o_sb_tx_req) begin
                ...
                `uvm_info("mbtrain_tx_txselfcal", $sformatf("Mismatch in o_tx_info: expected %0h, got %0h", o_tx_info_expected, item_tx_fsm_sb_out.o_tx_info), UVM_LOW)
                `uvm_info("mbtrain_tx_txselfcal", $sformatf("Mismatch in o_sb_tx_req: expected %0b, got %0b", o_sb_tx_req_expected, item_tx_fsm_sb_out.o_sb_tx_req), UVM_LOW)
            end
        end
        return match;
    endfunction
