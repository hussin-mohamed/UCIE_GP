    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_sequence_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        if(item_rx_fsm_sb_in.i_rx_decoding == RX_MBTRAIN_VALTRAINVREF_Start_Handshake && item_rx_fsm_sb_in.i_sb_rx_req==1'b1 && cntxt.currentstate_rx == mbtrain_rx_valtraincenter::instance())begin
            ...
            if (o_rx_encoding_expected==item_rx_fsm_sb_out.o_rx_encoding && o_rx_info_expected==item_rx_fsm_sb_out.o_rx_info && o_sb_rx_rsp_expected == item_rx_fsm_sb_out.o_sb_rx_rsp) begin
                ...
                `uvm_info("mbtrain_rx_valtrainvref", $sformatf("o_sb_rx_rsp mismatch expected value: %0b, got %0b", o_sb_rx_rsp_expected, item_rx_fsm_sb_out.o_sb_rx_rsp), UVM_LOW)
        ...
        else if((item_rx_fsm_sb_in.i_sb_rx_done == 1 && cntxt.currentstate_rx == mbtrain_rx_valtrainvref::instance()) || 
        (item_tx_fsm_sb_in.i_tx_decoding == Data_To_Clock_sweep_TX_End_Init_Handshake && item_controllers_in.i_rx_error==1'b1 && cntxt.currentstate_rx == data_to_clock_sweep::instance()))begin
             o_rx_encoding_expected = RX_MBTRAIN_VALTRAINVREF_Data_to_Clock_Test;
        ...
        else if (item_controllers_in.i_tx_done && item_controllers_in.i_rx_done && item_rx_fsm_sb_in.i_sb_rx_req && cntxt.currentstate_rx == mbtrain_rx_valtrainvref::instance()) begin
