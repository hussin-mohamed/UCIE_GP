    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_sequence_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        if(item_tx_fsm_sb_in.i_tx_decoding == MBTRAIN_RXCLKCAL_TX_End_Handshake && state_done && item_tx_fsm_sb_in.i_sb_tx_rsp==1'b1 && cntxt.currentstate_tx == mbtrain_tx_rxclkcal::Instance())begin
            ...
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_sb_tx_rsp_expected == item_tx_fsm_sb_out.o_sb_tx_rsp &&  o_error_threshhold_expected==item_controllers_out.o_error_threshhold) begin
                ...
                `uvm_info("mbtrain_tx_valtraincenter", $sformatf("Mismatch in o_tx_encoding: expected %0h, got %0h", o_tx_encoding_expected, item_tx_fsm_sb_out.o_tx_encoding), UVM_LOW)
                `uvm_info("mbtrain_tx_valtraincenter", $sformatf("o_tx_info mismatch expected value: %0h, got %0h", o_tx_info_expected, item_tx_fsm_sb_out.o_tx_info), UVM_LOW)
                `uvm_info("mbtrain_tx_valtraincenter", $sformatf("o_sb_tx_rsp_expected mismatch expected value: %0b, got %0b", o_sb_tx_rsp_expected, item_tx_fsm_sb_out.o_sb_tx_rsp), UVM_LOW)
                `uvm_info("mbtrain_tx_valtraincenter", $sformatf("o_error_threshhold_expected mismatch expected value: %0d, got %0d", o_error_threshhold_expected, item_controllers_out.o_error_threshhold), UVM_LOW)
            end
        end
