...
class mbtrain_tx_rxdeskew extends State;
    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_seq_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        if(item_tx_fsm_sb_in.i_tx_decoding == MBTRAIN_DATATRAINVREF_TX_End_Handshake && state_done && item_tx_fsm_sb_in.i_sb_tx_rsp==1'b1 && cntxt.currentstate_tx == mbtrain_tx_datatrainvref::Instance()  )begin
            ...
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding ) begin
                ...
                // `uvm_info("mbtrain_tx_rxdeskew", $sformatf("o_tx_sb_req mismatch expected value: %0b, got %0b", o_tx_sb_req_expected, item_tx_fsm_sb_out.o_tx_sb_req), UVM_LOW)
            end
        end
        else if (o_tx_encoding_expected == MBTRAIN_RXDESKEW_TX_Start_Handshake) begin
        ...
        else if (o_tx_encoding_expected == MBTRAIN_RXDESKEW_TX_End_Handshake) begin
             if (o_tx_encoding_expected == item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected == item_tx_fsm_sb_out.o_tx_info && o_tx_sb_req_expected == item_tx_fsm_sb_out.o_tx_sb_req) begin
                match=1;
            end else begin
                match =0;
