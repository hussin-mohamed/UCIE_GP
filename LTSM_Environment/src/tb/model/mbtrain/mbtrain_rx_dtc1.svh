...
class mbtrain_rx_dtc1 extends State;
    ...
    static function mbtrain_rx_dtc1 Instance();
        if (inst == null)
        inst = new();
        return inst;
    endfunction
    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_seq_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        // ----------------------------------------------------------------
        if(item_rx_fsm_sb_in.i_rx_decoding == RX_MBTRAIN_DTC1_Start_Handshake && item_rx_fsm_sb_in.i_sb_rx_req==1'b1 ) begin
        ...
        // ----------------------------------------------------------------
        else if (o_rx_encoding_expected == RX_MBTRAIN_DTC1_Start_Handshake) begin
            if (o_rx_encoding_expected==item_rx_fsm_sb_out.o_rx_encoding && o_rx_info_expected==item_rx_fsm_sb_out.o_rx_info && o_rx_sb_rsp_expected == item_rx_fsm_sb_out.o_rx_sb_rsp) begin
                ...
                match = 0;
                `uvm_info("mbtrain_rx_dtc1", $sformatf("Mismatch in o_rx_encoding: expected %0h, got %0h", o_rx_encoding_expected, item_rx_fsm_sb_out.o_rx_encoding), UVM_LOW)
                `uvm_info("mbtrain_rx_dtc1", $sformatf("o_rx_sb_rsp mismatch expected value: %0b, got %0b", o_rx_sb_rsp_expected, item_rx_fsm_sb_out.o_rx_sb_rsp), UVM_LOW)
        ...
        // ----------------------------------------------------------------
            ...
            if (o_rx_encoding_expected==item_rx_fsm_sb_out.o_rx_encoding && o_rx_info_expected==item_rx_fsm_sb_out.o_rx_info && o_rx_sb_req_expected == item_rx_fsm_sb_out.o_rx_sb_req && o_rx_data_expected == item_rx_fsm_sb_out.o_rx_data) begin
                ...
                match = 0;
                `uvm_info("mbtrain_rx_dtc1", $sformatf("Mismatch in o_rx_encoding: expected %0h, got %0h", o_rx_encoding_expected, item_rx_fsm_sb_out.o_rx_encoding), UVM_LOW)
                `uvm_info("mbtrain_rx_dtc1", $sformatf("o_rx_sb_req mismatch expected value: %0b, got %0b", o_rx_sb_req_expected, item_rx_fsm_sb_out.o_rx_sb_req), UVM_LOW)
                `uvm_info("mbtrain_rx_dtc1", $sformatf("o_rx_info mismatch expected value: %0h, got %0h", o_rx_info_expected, item_rx_fsm_sb_out.o_rx_info), UVM_LOW)
                `uvm_info("mbtrain_rx_dtc1", $sformatf("o_rx_data mismatch expected value: %0h, got %0h", o_rx_data_expected, item_rx_fsm_sb_out.o_rx_data), UVM_LOW)
            end 
            o_rx_encoding_expected =0;
        end
        // ----------------------------------------------------------------
        ...
        // encoding.
        // ----------------------------------------------------------------
        else if (o_rx_encoding_expected == RX_MBTRAIN_DTC1_End_Handshake) begin
            state_done = 1'b1;
            if (o_rx_encoding_expected == item_rx_fsm_sb_out.o_rx_encoding && o_rx_info_expected == item_rx_fsm_sb_out.o_rx_info && o_rx_sb_rsp_expected == item_rx_fsm_sb_out.o_rx_sb_rsp) begin
                match = 1;
            end else begin
    ...
    virtual function fsm_t getStateId();
        return fsm_mbtrain_rx_dtc1;
    endfunction


endclass //mbtrain_rx_dtc1 extends state