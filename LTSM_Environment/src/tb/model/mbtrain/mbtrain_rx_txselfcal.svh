...
class mbtrain_rx_txselfcal extends State;
    local static mbtrain_rx_txselfcal inst = null;
    ...
    static function mbtrain_rx_txselfcal Instance();
        if (inst == null)
        inst = new();
        return inst;
    endfunction
    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_seq_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        if( item_rx_fsm_sb_in.i_rx_decoding == 'hd0 && item_rx_fsm_sb_in.i_sb_rx_req==1'b1 )begin
            o_rx_encoding_expected = RX_MBTRAIN_TXSELFCAL_End_Handshake-1;
            ...
            if (o_rx_encoding_expected==item_rx_fsm_sb_out.o_rx_encoding ) begin
                ...
                match = 0;
                `uvm_info("mbtrain_rx_txselfcal", $sformatf("Mismatch in o_rx_encoding: expected %0h, got %0h", o_rx_encoding_expected, item_rx_fsm_sb_out.o_rx_encoding), UVM_LOW)
                // `uvm_info("mbtrain_rx_txselfcal", $sformatf("o_rx_info mismatch expected value: %0h, got %0h", o_rx_info_expected, item_rx_fsm_sb_out.o_rx_info), UVM_LOW)
                // `uvm_info("mbtrain_rx_txselfcal", $sformatf("o_rx_sb_req mismatch expected value: %0b, got %0b", o_rx_sb_rsp_expected, item_rx_fsm_sb_out.o_rx_sb_rsp), UVM_LOW)
            end
        end
        else if (o_rx_encoding_expected == RX_MBTRAIN_TXSELFCAL_End_Handshake-1) begin
            state_done=1'b1;
    ...
    virtual function fsm_t getStateId();
        return fsm_mbtrain_rx_txselfcal;
    endfunction
endclass //mbtrain_rx_txselfcal extends state