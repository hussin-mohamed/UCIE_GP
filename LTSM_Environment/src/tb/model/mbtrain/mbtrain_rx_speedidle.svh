...
class mbtrain_rx_speedidle extends State;
    local static mbtrain_rx_speedidle inst = null;
   ...
   protected function new(); endfunction
    ...
    static function mbtrain_rx_speedidle Instance();
        if (inst == null)
        inst = new();
        return inst;
    endfunction
    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_seq_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        if (cntxt.currentstate_rx!=mbtrain_rx_speedidle::Instance()) begin
            rx_handshake_done =0;
            o_rx_encoding_expected = RX_MBTRAIN_SPEEDIDLE_Speed_Transition;
            if (o_rx_encoding_expected==item_rx_fsm_sb_out.o_rx_encoding) begin
                match = 1;
            end else begin
                match = 0;
                `uvm_info("mbtrain_rx_speedidle", $sformatf("Mismatch in o_rx_encoding: expected %0h, got %0h", o_rx_encoding_expected, item_rx_fsm_sb_out.o_rx_encoding), UVM_LOW)
            end
        end                                    
        ...
        else if (o_rx_encoding_expected == RX_MBTRAIN_SPEEDIDLE_End_Handshake) begin
            state_done=1'b1;
            if (o_rx_encoding_expected == item_rx_fsm_sb_out.o_rx_encoding && o_rx_info_expected == item_rx_fsm_sb_out.o_rx_info && o_rx_sb_rsp_expected == item_rx_fsm_sb_out.o_rx_sb_rsp) begin
                ...
                match =0;
                `uvm_info("mbtrain_rx_speedidle", $sformatf("Mismatch in o_rx_encoding: expected %0h, got %0h", o_rx_encoding_expected, item_rx_fsm_sb_out.o_rx_encoding), UVM_LOW)
                `uvm_info("mbtrain_rx_speedidle", $sformatf("o_rx_info mismatch expected value: %0h, got %0h", o_rx_info_expected, item_rx_fsm_sb_out.o_rx_info), UVM_LOW)
                `uvm_info("mbtrain_rx_speedidle", $sformatf("o_rx_sb_rsp mismatch expected value: %0b, got %0b", o_rx_sb_rsp_expected, item_rx_fsm_sb_out.o_rx_sb_rsp), UVM_LOW)
            end 
