...
class mbtrain_tx_dtc2 extends State;
    local static mbtrain_tx_dtc2 inst = null;
    ...
    static function mbtrain_tx_dtc2 Instance();
        if (inst == null)
        inst = new();
        return inst;
    endfunction
    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_seq_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        // ----------------------------------------------------------------
        // Phase 1 trigger: Previous state (rxdeskew) End Handshake response
        // received → enter dtc2 Start Handshake. Clear state_done,
        // arm retry=0, verify only o_tx_encoding this cycle.
        // ----------------------------------------------------------------
        if(item_tx_fsm_sb_in.i_tx_decoding == MBTRAIN_RXDESKEW_TX_End_Handshake && state_done && item_tx_fsm_sb_in.i_sb_tx_rsp==1'b1 && cntxt.currentstate_tx == mbtrain_tx_rxdeskew::Instance())begin
            o_tx_encoding_expected = MBTRAIN_DTC2_TX_Start_Handshake;
            enter_speeddegrade=1'b0;
            state_done=1'b0;
            ...
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding) begin
                ...
                match = 0;
                `uvm_info("mbtrain_tx_dtc2", $sformatf("Mismatch in o_tx_encoding: expected %0h, got %0h", o_tx_encoding_expected, item_tx_fsm_sb_out.o_tx_encoding), UVM_LOW)
            end
        end
        // ----------------------------------------------------------------
        // Phase 2 follow-up: Start Handshake still in progress.
        // Drive o_tx_sb_req low once i_sb_tx_done is seen, then do full
        // 3-field check (encoding, info, req) and clear expected encoding.
        // ----------------------------------------------------------------
        else if (o_tx_encoding_expected == MBTRAIN_DTC2_TX_Start_Handshake) begin
            o_tx_sb_req_expected = 1'b1;
            o_tx_info_expected = 16'h0000;
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_tx_sb_req_expected == item_tx_fsm_sb_out.o_tx_sb_req) begin
                ...
                match = 0;
                `uvm_info("mbtrain_tx_dtc2", $sformatf("Mismatch in o_tx_encoding: expected %0h, got %0h", o_tx_encoding_expected, item_tx_fsm_sb_out.o_tx_encoding), UVM_LOW)
                `uvm_info("mbtrain_tx_dtc2", $sformatf("o_tx_info mismatch expected value: %0h, got %0h", o_tx_info_expected, item_tx_fsm_sb_out.o_tx_info), UVM_LOW)
                `uvm_info("mbtrain_tx_dtc2", $sformatf("o_tx_sb_req mismatch expected value: %0b, got %0b", o_tx_sb_req_expected, item_tx_fsm_sb_out.o_tx_sb_req), UVM_LOW)
            end
        ...
        // ----------------------------------------------------------------
            ...
            o_tx_info_expected = 16'h0000;
             if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_tx_sb_rsp_expected == item_tx_fsm_sb_out.o_tx_sb_rsp /*&&  o_error_threshhold_expected==item_controllers_out.o_error_threshhold*/) begin
                ...
                `uvm_info("mbtrain_tx_dtc2", $sformatf("Mismatch in o_tx_encoding: expected %0h, got %0h", o_tx_encoding_expected, item_tx_fsm_sb_out.o_tx_encoding), UVM_LOW)
                `uvm_info("mbtrain_tx_dtc2", $sformatf("o_tx_info mismatch expected value: %0h, got %0h", o_tx_info_expected, item_tx_fsm_sb_out.o_tx_info), UVM_LOW)
                `uvm_info("mbtrain_tx_dtc2", $sformatf("o_tx_sb_rsp_expected mismatch expected value: %0b, got %0b", o_tx_sb_rsp_expected, item_tx_fsm_sb_out.o_tx_sb_rsp), UVM_LOW)
                //`uvm_info("mbtrain_tx_dtc2", $sformatf("o_error_threshhold_expected mismatch expected value: %0d, got %0d", o_error_threshhold_expected, item_controllers_out.o_error_threshhold), UVM_LOW)
            end
            o_tx_encoding_expected =0;
            first_time=1;
        end
        ...
        // ----------------------------------------------------------------
            ...
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding ) begin
                ...
                `uvm_info("mbtrain_tx_dtc2", $sformatf("Mismatch in o_tx_encoding: expected %0h, got %0h", o_tx_encoding_expected, item_tx_fsm_sb_out.o_tx_encoding), UVM_LOW)
                // `uvm_info("mbtrain_tx_dtc2", $sformatf("o_tx_info mismatch expected value: %0h, got %0h", o_tx_info_expected, item_tx_fsm_sb_out.o_tx_info), UVM_LOW)
                // `uvm_info("mbtrain_tx_dtc2", $sformatf("o_tx_sb_rsp mismatch expected value: %0b, got %0b", o_tx_sb_rsp_expected, item_tx_fsm_sb_out.o_tx_sb_rsp), UVM_LOW)
            end
        end
        // ----------------------------------------------------------------
        ...
        // ----------------------------------------------------------------
        else if (item_tx_fsm_sb_in.i_sb_tx_done && !train && !first ) begin
            o_tx_encoding_expected = MBTRAIN_DTC2_TX_End_Handshake;
            o_tx_info_expected = 16'h0000;
            o_tx_sb_req_expected =1'b1;
            first=1;
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding) begin
        ...
        // ----------------------------------------------------------------
        // Phase 2 follow-up: dtc2 End Handshake in progress.
        // Full 3-field check. No encoding reset — state machine transitions
        // out via an external mechanism.
        // ----------------------------------------------------------------
        else if (o_tx_encoding_expected == MBTRAIN_DTC2_TX_End_Handshake) begin
            o_tx_sb_req_expected =1'b1;
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_tx_sb_req_expected == item_tx_fsm_sb_out.o_tx_sb_req) begin
                match = 1;
            end else begin
    ...
    virtual function fsm_t getStateId();
        return fsm_mbtrain_tx_dtc2;
    endfunction


endclass //mbtrain_tx_dtc2 extends state