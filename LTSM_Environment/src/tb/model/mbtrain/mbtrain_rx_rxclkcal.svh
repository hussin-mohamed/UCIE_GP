    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_seq_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        if(item_rx_fsm_sb_in.i_rx_decoding == RX_MBTRAIN_RXCLKCAL_Start_Handshake && item_rx_fsm_sb_in.i_sb_rx_req==1'b1 && cntxt.currentstate_rx == mbtrain_rx_txselfcal::Instance())begin
            ...
            if (o_rx_encoding_expected==item_rx_fsm_sb_out.o_rx_encoding) begin
                ...
                // `uvm_info("mbtrain_rx_rxclkcal", $sformatf("o_rx_sb_req mismatch expected value: %0b, got %0b", o_rx_sb_rsp_expected, item_rx_fsm_sb_out.o_rx_sb_rsp), UVM_LOW)
            end
        end
        else if (o_rx_encoding_expected == RX_MBTRAIN_RXCLKCAL_Start_Handshake) begin
            ...
            o_rx_encoding_expected = MBTRAIN_RXCLKCAL_TX_End_Handshake;
            o_rx_info_expected = 16'h0000;
            o_rx_sb_rsp_expected = 1'b1;
            if (o_rx_encoding_expected==item_rx_fsm_sb_out.o_rx_encoding) begin
                match = 1;
        ...
        else if (o_rx_encoding_expected == MBTRAIN_RXCLKCAL_TX_End_Handshake) begin
            if (o_rx_encoding_expected==item_rx_fsm_sb_out.o_rx_encoding && o_rx_info_expected==item_rx_fsm_sb_out.o_rx_info && o_rx_sb_rsp_expected == item_rx_fsm_sb_out.o_rx_sb_rsp) begin
                match = 1;
            end else begin
                match = 0;
