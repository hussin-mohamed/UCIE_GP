...
class mbtrain_tx_dtc1 extends State;
    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_seq_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        // ----------------------------------------------------------------
        // Phase 1 trigger: Previous state (valtrainvref) End Handshake response
        // received → enter dtc1 Start Handshake. Clear state_done,
        ...
        // ----------------------------------------------------------------
        else if (o_tx_encoding_expected == MBTRAIN_DTC1_TX_Start_Handshake) begin
            o_tx_sb_req_expected = 1'b1;
            o_tx_info_expected = 16'h0000;
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_tx_sb_req_expected == item_tx_fsm_sb_out.o_tx_sb_req) begin
                ...
                `uvm_info("mbtrain_tx_dtc1", $sformatf("o_tx_sb_req mismatch expected value: %0b, got %0b", o_tx_sb_req_expected, item_tx_fsm_sb_out.o_tx_sb_req), UVM_LOW)
            end
        ...
        // ----------------------------------------------------------------
            ...
            o_tx_info_expected = 16'h0000;
             if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_tx_sb_rsp_expected == item_tx_fsm_sb_out.o_tx_sb_rsp /*&&  o_error_threshhold_expected==item_controllers_out.o_error_threshhold*/) begin
                ...
                `uvm_info("mbtrain_tx_dtc1", $sformatf("Mismatch in o_tx_encoding: expected %0h, got %0h", o_tx_encoding_expected, item_tx_fsm_sb_out.o_tx_encoding), UVM_LOW)
                `uvm_info("mbtrain_tx_dtc1", $sformatf("o_tx_info mismatch expected value: %0h, got %0h", o_tx_info_expected, item_tx_fsm_sb_out.o_tx_info), UVM_LOW)
                `uvm_info("mbtrain_tx_dtc1", $sformatf("o_tx_sb_rsp_expected mismatch expected value: %0b, got %0b", o_tx_sb_rsp_expected, item_tx_fsm_sb_out.o_tx_sb_rsp), UVM_LOW)
        ...
        // ----------------------------------------------------------------
            ...
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding ) begin
                ...
                `uvm_info("mbtrain_tx_dtc1", $sformatf("Mismatch in o_tx_encoding: expected %0h, got %0h", o_tx_encoding_expected, item_tx_fsm_sb_out.o_tx_encoding), UVM_LOW)
                // `uvm_info("mbtrain_tx_dtc1", $sformatf("o_tx_info mismatch expected value: %0h, got %0h", o_tx_info_expected, item_tx_fsm_sb_out.o_tx_info), UVM_LOW)
                // `uvm_info("mbtrain_tx_dtc1", $sformatf("o_tx_sb_rsp mismatch expected value: %0b, got %0b", o_tx_sb_rsp_expected, item_tx_fsm_sb_out.o_tx_sb_rsp), UVM_LOW)
            end
        end
        // ----------------------------------------------------------------
        ...
        // ----------------------------------------------------------------
            ...
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_tx_sb_rsp_expected == item_tx_fsm_sb_out.o_tx_sb_rsp) begin
                match = 1;
            end else begin
                match = 0;
                `uvm_info("mbtrain_tx_dtc1", $sformatf("Mismatch in o_tx_encoding: expected %0h, got %0h", o_tx_encoding_expected, item_tx_fsm_sb_out.o_tx_encoding), UVM_LOW)
        ...
        // ----------------------------------------------------------------
        // Phase 1 trigger: TX done after END INIT (not training, first not yet set)
        // → transition to dtc1 End Handshake. Sets first=1 to prevent re-entry,
        // asserts o_tx_sb_req, verifies only encoding this cycle.
        // ----------------------------------------------------------------
        ...
        // ----------------------------------------------------------------
        // Phase 2 follow-up: dtc1 End Handshake in progress.
        // Full 3-field check. No encoding reset — state machine transitions
        // out via an external mechanism.
        // ----------------------------------------------------------------
        else if (o_tx_encoding_expected == MBTRAIN_DTC1_TX_End_Handshake) begin
            o_tx_sb_req_expected =1'b1;
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_tx_sb_req_expected == item_tx_fsm_sb_out.o_tx_sb_req) begin
                match = 1;
            end else begin
