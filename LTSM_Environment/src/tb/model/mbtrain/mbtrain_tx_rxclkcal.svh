...
class mbtrain_tx_rxclkcal extends State;
    local static mbtrain_tx_rxclkcal inst = null;
    ...
    static function mbtrain_tx_rxclkcal Instance();
        if (inst == null)
        inst = new();
        return inst;
    endfunction
    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_seq_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        if(item_tx_fsm_sb_in.i_tx_decoding == 'hd1 && state_done && item_tx_fsm_sb_in.i_sb_tx_rsp==1'b1 && cntxt.currentstate_tx == mbtrain_tx_txselfcal::Instance() )begin
            ...
            if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding ) begin
                ...
                // `uvm_info("mbtrain_tx_rxclkcal", $sformatf("o_tx_sb_req mismatch expected value: %0b, got %0b", o_tx_sb_req_expected, item_tx_fsm_sb_out.o_tx_sb_req), UVM_LOW)
            end
        end
        else if (o_tx_encoding_expected == MBTRAIN_RXCLKCAL_TX_Start_Handshake) begin
        ...
        else if((item_tx_fsm_sb_in.i_tx_decoding == MBTRAIN_RXCLKCAL_TX_Start_Handshake && item_tx_fsm_sb_in.i_sb_tx_rsp==1'b1 ))begin
        ...
        else if (o_tx_encoding_expected == MBTRAIN_RXCLKCAL_TX_End_Handshake) begin
             if (o_tx_encoding_expected==item_tx_fsm_sb_out.o_tx_encoding && o_tx_info_expected==item_tx_fsm_sb_out.o_tx_info && o_tx_sb_req_expected == item_tx_fsm_sb_out.o_tx_sb_req) begin
                match = 1;
            end else begin
                match = 0;
    ...
    virtual function fsm_t getStateId();
        return fsm_mbtrain_tx_rxclkcal;
    endfunction


endclass //mbtrain_tx_rxclkcal extends state