...
class mbtrain_rx_datatrainvref extends State;
    ...
    bit match;
    bit first;       // Set in Start Handshake Phase 2; enables the rx_done→INIT trigger
    bit first_time;  // Set in LFSR CLEAR Phase 1; enables the rx_done→PATTERN GEN trigger
    bit firstt;      // Set when end_sweep fires; guards the END INIT trigger
    ...
    virtual function bit doSpecificCombAction(FSMContext cntxt,LTSM_controllers_seq_item item_controllers_in,ltsm_rdi_sequence_item item_rdi_in,rx_fsm_sb_sequence_item item_rx_fsm_sb_in,tx_fsm_sb_sequence_item item_tx_fsm_sb_in,
        ...
        // request from mbtrain_rx_dtc1. Reset end_sweep and guards,
        // arm train, set rsp. Verify only o_rx_encoding this cycle.
        // ----------------------------------------------------------------
        ...
        // encoding and set first=1 to enable the rx_done→INIT trigger.
        // ----------------------------------------------------------------
        else if (o_rx_encoding_expected == RX_MBTRAIN_DATATRAINVREF_Start_Handshake) begin
            if (o_rx_encoding_expected == item_rx_fsm_sb_out.o_rx_encoding && o_rx_info_expected == item_rx_fsm_sb_out.o_rx_info && o_rx_sb_rsp_expected == item_rx_fsm_sb_out.o_rx_sb_rsp) begin
                ...
                `uvm_info("mbtrain_rx_datatrainvref", $sformatf("o_rx_sb_rsp mismatch expected value: %0b, got %0b", o_rx_sb_rsp_expected, item_rx_fsm_sb_out.o_rx_sb_rsp), UVM_LOW)
            end
        ...
        // ----------------------------------------------------------------
            ...
            if (o_rx_encoding_expected==item_rx_fsm_sb_out.o_rx_encoding && o_rx_info_expected==item_rx_fsm_sb_out.o_rx_info && o_rx_sb_req_expected == item_rx_fsm_sb_out.o_rx_sb_req && o_rx_data_expected == item_rx_fsm_sb_out.o_rx_data) begin
                ...
                match = 0;
                `uvm_info("mbtrain_rx_datatrainvref", $sformatf("Mismatch in o_rx_encoding: expected %0h, got %0h", o_rx_encoding_expected, item_rx_fsm_sb_out.o_rx_encoding), UVM_LOW)
                `uvm_info("mbtrain_rx_datatrainvref", $sformatf("o_rx_sb_req mismatch expected value: %0b, got %0b", o_rx_sb_req_expected, item_rx_fsm_sb_out.o_rx_sb_req), UVM_LOW)
                `uvm_info("mbtrain_rx_datatrainvref", $sformatf("o_rx_info mismatch expected value: %0h, got %0h", o_rx_info_expected, item_rx_fsm_sb_out.o_rx_info), UVM_LOW)
                `uvm_info("mbtrain_rx_datatrainvref", $sformatf("o_rx_data mismatch expected value: %0h, got %0h", o_rx_data_expected, item_rx_fsm_sb_out.o_rx_data), UVM_LOW)
            end 
            o_rx_encoding_expected =0;
        end
        // ----------------------------------------------------------------
        ...
        // ----------------------------------------------------------------
        else if (item_rx_fsm_sb_in.i_sb_rx_req && item_rx_fsm_sb_in.i_rx_decoding == RX_MBTRAIN_DATATRAINVREF_End_Handshake) begin
        ...
        // encoding.
        // ----------------------------------------------------------------
        else if (o_rx_encoding_expected == RX_MBTRAIN_DATATRAINVREF_End_Handshake) begin
            if (o_rx_encoding_expected == item_rx_fsm_sb_out.o_rx_encoding && o_rx_info_expected == item_rx_fsm_sb_out.o_rx_info && o_rx_sb_rsp_expected == item_rx_fsm_sb_out.o_rx_sb_rsp) begin
                match = 1;
            end else begin
                match = 0;
