...
// CLASS: ltsm_mbinit_reversalmb_fail_v_seqeunce
//
...
class ltsm_mbinit_reversalmb_fail_v_seqeunce extends virtual_sequence_base;

  `uvm_object_utils(ltsm_mbinit_reversalmb_fail_v_seqeunce)

  function new(string name = "ltsm_mbinit_reversalmb_fail_v_seqeunce");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    ltsm_mbinit_reversal_result_exit_tx#("fail") res_exit_tx;
    ...
    fork
      begin
        ...
        // result exit
        // looping in the resvesal faliure case till the trainerror transition happens
        repeat(1000) begin
            res_exit_tx = ltsm_mbinit_reversal_result_exit_tx#("fail")::type_id::create("res_exit_tx");
