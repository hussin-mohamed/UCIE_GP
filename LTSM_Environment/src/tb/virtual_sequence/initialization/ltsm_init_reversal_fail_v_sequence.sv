...
//------------------------------------------------------------------------------
//
// CLASS: ltsm_init_reversal_fail_v_sequence
//
// this virtual sequence is used to mimic the initialization flow with reversal fail.
// 
// 
//
//------------------------------------------------------------------------------

class ltsm_init_reversal_fail_v_sequence extends virtual_sequence_base;
    `uvm_object_utils(ltsm_init_reversal_fail_v_sequence)
    ...
    ltsm_mbinit_reversalmb_fail_v_seqeunce  reversal;

    // Function: new
    //
    // Creates a new virtual_sequence instance with the given name.

    extern function new(string name = "ltsm_init_reversal_fail_v_sequence");
...
endclass : ltsm_init_reversal_fail_v_sequence
...
function ltsm_init_reversal_fail_v_sequence::new(string name = "ltsm_init_reversal_fail_v_sequence");
    super.new(name);
endfunction : new
...
task ltsm_init_reversal_fail_v_sequence::pre_body();
    // tx sequences
    ...
    reversal=ltsm_mbinit_reversalmb_fail_v_seqeunce::type_id::create("reversal");
endtask

// body
// ----

task ltsm_init_reversal_fail_v_sequence::body();
    super.body();
    reset.start(v_seqr);
    sbinit.start(v_seqr);
    param.start(v_seqr);
