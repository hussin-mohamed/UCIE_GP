...
module ucie_ltsm_rx_mbinit_cal #(
  ...
  always_ff @(posedge i_clk or posedge i_reset) begin
    if (i_reset) begin
      ...
      if (current_substate == DONE_HANDSHAKE && i_sb_rx_req && i_rx_decoding == 9'h18)
        substates_done <= 1;
    end
  end
...
`ifdef SIM
  ...
  endproperty
  ENC_DONE_HANDSHAKE :
  assert property (enc_done_handshake)
  ...
  property done_only_in_state;
    @(posedge i_clk) disable iff (i_reset) i_current_state != MBINIT_CAL |-> !o_done_mbinit_cal_rx;
  endproperty
  DONE_ONLY_IN_STATE :
