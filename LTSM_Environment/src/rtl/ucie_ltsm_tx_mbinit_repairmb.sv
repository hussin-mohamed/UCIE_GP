module ucie_ltsm_tx_mbinit_repairmb #(
  ...
  // logic                      train_error_data_to_clock_test_reg;
  logic                      no_retry;
  logic                      init;
  logic                      failed_test;
  ...
  // always_ff @(posedge i_clk or posedge i_reset) begin
  //   if (i_reset) train_error_data_to_clock_test_reg <= 0;
  //   else train_error_data_to_clock_test_reg <= train_error_data_to_clock_test;
  // end
    ...
    if (!substates_done && (o_timer_8ms)) begin
      ...
      case (current_substate)
        ...
        DATA_TO_CLOCK_TEST: begin
          ...
          if (!substates_done) begin
            if (clock_to_test_done) next_substate = APPLY_DEGRADE;
  ...
  always_ff @(posedge i_clk or posedge i_reset) begin
    if (i_reset) o_train_error <= 1'b0;
    else o_train_error <= o_train_error_internal | train_error_data_to_clock_test;
  end
...
`ifdef SIM
  ...
  ENC_INIT_HANDSHAKE :
  assert property (encoding_check(INIT_HANDSHAKE, 9'h38))
  else $error("ASSERT FAIL [ENC_INIT_HANDSHAKE]");
  ENC_APPLY_DEGRADE :
  assert property (encoding_check(APPLY_DEGRADE, 9'h3A))
  else $error("ASSERT FAIL [ENC_APPLY_DEGRADE]");
  ENC_DONE_HANDSHAKE :
  assert property (encoding_check(DONE_HANDSHAKE, 9'h3B))
  else $error("ASSERT FAIL [ENC_DONE_HANDSHAKE]");

  property timeout_sets_train_error;
    @(posedge i_clk) disable iff (i_reset) o_timer_8ms |-> o_train_error_internal;
  endproperty
  TIMEOUT_TRAIN_ERROR :
  assert property (timeout_sets_train_error)
  ...
  property sweep_error_propagates;
    @(posedge i_clk) disable iff (i_reset) train_error_data_to_clock_test |-> o_train_error;
  endproperty
  SWEEP_TRAIN_ERROR :
  assert property (sweep_error_propagates)
  ...
  property degrade_not_possible_error;
    @(posedge i_clk) disable iff (i_reset || o_timer_8ms)
        ...
        |-> o_train_error;
  endproperty
  DEGRADE_NOT_POSSIBLE_ERROR :
  assert property (degrade_not_possible_error)
  ...
  endproperty
  MISMATCH_RERUNS :
  assert property (mismatch_reruns_sweep)
  ...
  endproperty
  MATCH_ADVANCES :
  assert property (match_advances_to_done)
  ...
  endproperty
  SWEEP_ENABLED :
  assert property (sweep_enabled_in_test)
