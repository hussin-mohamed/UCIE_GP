...
class ltsm_rdi_agent extends uvm_agent;
    ...
    uvm_analysis_port #(ltsm_rdi_sequence_item) drvr_ap, mntr_ap;
    ltsm_rdi_driver drvr;
    ltsm_rdi_monitor mntr;
...
function void ltsm_rdi_agent::build_phase(uvm_phase phase);
    ...
    mntr = ltsm_rdi_monitor::type_id::create("mntr", this);

    if (rdi_cfg != null) is_active = rdi_cfg.is_active;

    if(is_active == UVM_ACTIVE) begin
        drvr = ltsm_rdi_driver::type_id::create("drvr", this);
...
function void ltsm_rdi_agent::connect_phase(uvm_phase phase);
    ...
    mntr.vif = rdi_cfg.bfm;

    if(is_active == UVM_ACTIVE) begin
        drvr.seq_item_port.connect(seqr.seq_item_export);
        drvr.bfm = rdi_cfg.bfm;
