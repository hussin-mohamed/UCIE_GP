...
function void sb_test_base::build_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(virtual sb_ltsm_ctrl_vbfm)::get(this, "", "ltsm_ctrl_vbfm", env_cfg.ltsm_ctrl_if))
        `uvm_fatal("build_phase", "TEST - Unable to get the ltsm_ctrl_vbfm from the uvm_config_db")

    if(!uvm_config_db#(virtual sb_tx_path_vbfm)::get(this,   "", "tx_path_vbfm",   env_cfg.tx_path_if))
        `uvm_fatal("build_phase", "TEST - Unable to get the tx_path_vbfm from the uvm_config_db")

    if(!uvm_config_db#(virtual sb_rx_path_vbfm)::get(this,   "", "rx_path_vbfm",   env_cfg.rx_path_if))
        `uvm_fatal("build_phase", "TEST - Unable to get the rx_path_vbfm from the uvm_config_db")

    if(!uvm_config_db#(virtual sb_rdi_vbfm)::get(this,       "", "rdi_vbfm",       env_cfg.rdi_if))
        `uvm_fatal("build_phase", "TEST - Unable to get the rdi_vbfm from the uvm_config_db")

    if(!uvm_config_db#(virtual sb_phy_link_vbfm)::get(this,  "", "phy_link_vbfm",  env_cfg.phy_link_if))
