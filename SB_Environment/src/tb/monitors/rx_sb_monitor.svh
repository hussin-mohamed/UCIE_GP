...
class rx_sb_monitor extends uvm_monitor;
    `uvm_component_utils(rx_sb_monitor)
    rx_sequence_item item;

    virtual sb_ltsm_rx_bfm vif;
    uvm_analysis_port #(rx_sequence_item) ap;

   // Function: new
    //
    // Creates a new rx_sb_monitor instance with the given name and parent.

    extern function new(string name = "rx_sb_monitor", uvm_component parent = null);
...
function rx_sb_monitor::new(string name = "rx_sb_monitor", uvm_component parent = null);
...
function rx_sb_monitor::build_phase(uvm_phase phase);
...
task rx_sb_monitor::run_phase(rx_sequence_item item);
    super.run_phase(phase);
    forever begin
        @(negedge vif.clk);
        item = rx_sequence_item::type_id::create("item");
        item.data=vif.o_rx_data;
        item.encoding=vif.o_rx_encoding;
        item.info=vif.o_rx_info;
        ap.write(item); 
    end
endtask
