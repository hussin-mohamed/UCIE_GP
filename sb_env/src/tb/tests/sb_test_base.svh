...
class sb_test_base extends uvm_test;
  ...
  //
  // Base test class for the Sideband testbench, handling environment setup and common test configuration.
...
function void sb_test_base::end_of_elaboration_phase(uvm_phase phase);
  ...
  vseq = sb_sanity_vseq::type_id::create("vseq", this);
...
task sb_test_base::main_phase(uvm_phase phase);
  fork
    begin
  ...
  if(hit_reset) begin
    ...
    `uvm_info(get_type_name(), $sformatf("Jumping backward to the uvm_pre_reset_phase after %0dns delayBase test class for the Sideband testbench, handling environment setup and common test configuration.", reset_delay_ns), UVM_LOW)
    #(reset_delay_ns);
    phase.drop_objection(this);
    phase.get_objection().set_report_severity_id_override(UVM_WARNING, "OBJTN_CLEAR", UVM_INFO);
    hit_reset = 0;
...
function void sb_test_base::phase_ready_to_end(uvm_phase phase);
