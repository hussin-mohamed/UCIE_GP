...
//---------------------------------------------------------------------------
//
// CLASS: glitch_neg_pat_gen_test
//
// SVAUnit negative test that injects a one-cycle glitch into the quiet period
// of the local-die TX pattern generation.
//
//---------------------------------------------------------------------------
...
class glitch_neg_pat_gen_test extends svaunit_test;
  ...
  // Function: new
  //
  // Creates the glitch_neg_pat_gen_test component.

  extern function new(string name = "glitch_neg_pat_gen_test", uvm_component parent);
  ...
  //
  // Drives one TX pattern iteration and optionally injects a quiet-period glitch.

  extern task tx_pattern_iteration(bit inject_glitch = 0);
  ...
  // Task: test
  //
  // Runs the negative scenario where the final TX iteration contains a glitch.
...
function glitch_neg_pat_gen_test::new(string name = "glitch_neg_pat_gen_test", uvm_component parent);
...
function void glitch_neg_pat_gen_test::build_phase(uvm_phase phase);
...
task glitch_neg_pat_gen_test::tx_pattern_iteration(bit inject_glitch = 0);
...
task glitch_neg_pat_gen_test::rx_pattern_iteration();
  for (int i = 0; i < 64; i++) begin
    @(posedge vif.clk_1x);
...
task glitch_neg_pat_gen_test::test();
