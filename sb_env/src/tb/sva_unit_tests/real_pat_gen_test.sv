...
//---------------------------------------------------------------------------
//
// CLASS: real_pat_gen_test
//
// SVAUnit test that models the full time-multiplexed TX/RX pattern-generation
// behavior, including pause windows, asynchronous detection, and ready
// handshaking.
//
//---------------------------------------------------------------------------
...
class real_pat_gen_test extends svaunit_test;
  ...
  // Function: new
  //
  // Creates the real_pat_gen_test component.

  extern function new(string name = "real_pat_gen_test", uvm_component parent);
  ...
  // Task: test
  //
  // Runs the full pattern-generation scenario with asynchronous detection and
  // time-windowed TX generation.
...
function real_pat_gen_test::new(string name = "real_pat_gen_test", uvm_component parent);
...
function void real_pat_gen_test::build_phase(uvm_phase phase);
...
function logic real_pat_gen_test::get_pattern_bit(int idx);
  if (idx < 64) begin
...
task real_pat_gen_test::rx_pattern_iteration();
