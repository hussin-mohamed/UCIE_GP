module ucie_sb_tx_path (
  ...
  always_comb begin
    ...
    case (current_state)
      IDLE: begin
        if (i_sb_init_start) next_state = CYCLING;
      end
      ...
      EXTRA_ITERS: begin
        if (iter_counter == 4) next_state = DONE;
      end

      DONE: begin
        if (!i_sb_init_start) next_state = IDLE;
      end
    endcase
  end
  ...
  always_ff @(posedge i_s_clk or posedge i_reset) begin
    if (i_reset) begin
      ...
      if (w_pattern_running) begin
        if (ui_counter == 95) ui_counter <= 0;
        else ui_counter <= ui_counter + 1;
      end else begin
        if (current_state_serdes == IDLE || current_state_serdes == DONE) ui_counter <= 0;
      end

      if (current_state_serdes == IDLE) begin
        iter_counter <= 0;
      end else if (current_state_serdes == EXTRA_ITERS) begin
        if (ui_counter == 95) iter_counter <= iter_counter + 1;
      end
    end
  end
  ...
  always_ff @(negedge i_s_clk or posedge i_reset) begin
    if (i_reset) r_clk_en_safe <= 1'b1;
    else if (ui_counter >= 63 && ui_counter <= 65) r_clk_en_safe <= 1'b0;
    else r_clk_en_safe <= 1'b1;
  end
