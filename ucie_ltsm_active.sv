...
module ucie_ltsm_active (
    ...
    // Valid Framing error
    input  logic        i_rx_valid_results,

    // 2us timer signal for PMNACK
    ...
    always_comb begin
        ...
        if (i_current_state == ACTIVE || i_current_state == ACTIVE_PMNAK) begin
            ...
            // valid framing error detected --> go to PHYRETRAIN
            end else if(i_rx_valid_results == 0) begin 
                o_done_active_retrain = 1'b1;

            // RDI State Change Requests
            end else begin
                case (i_lp_state_req)
